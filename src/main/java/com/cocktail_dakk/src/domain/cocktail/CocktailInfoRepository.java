...
public interface CocktailInfoRepository extends JpaRepository<CocktailInfo, Long> {
    @Query("SELECT c FROM CocktailInfo c join c.cocktailKeywords ck join ck.keyword k where c.koreanName like concat('%', :koreanName, '%')  or c.englishName like concat('%', :englishName, '%') or c.ingredient like concat('%', :ingredients, '%') or k.keywordName like concat('%', :keywordName, '%')")
